...
/* ── Neural Canvas (partículas douradas) ── */
...
class N {
  ...
  draw() {
    ...
    const a = this.act ? .5 + p * .35 : .12 + p * .14;
    const s = this.r * (this.act ? 1 + p * .4 : 1);
    cx.beginPath(); cx.arc(this.x, this.y, s, 0, Math.PI * 2);
    cx.fillStyle = this.act ? `rgba(201,168,76,${a})` : `rgba(120,85,15,${a * .7})`; cx.fill();
    if (this.act && p > .68) {
      cx.beginPath(); cx.arc(this.x, this.y, s * 3, 0, Math.PI * 2);
      cx.fillStyle = `rgba(201,168,76,${(p - .68) * .12})`; cx.fill();
    }
  }
}
...
function drawC() {
  for (let i = 0; i < nodes.length; i++) for (let j = i + 1; j < nodes.length; j++) {
    ...
    if (d < 130) {
      const a = (1 - d / 130) * .22, ac = nodes[i].act || nodes[j].act;
      cx.beginPath(); cx.moveTo(nodes[i].x, nodes[i].y); cx.lineTo(nodes[j].x, nodes[j].y);
      cx.strokeStyle = ac ? `rgba(201,168,76,${a})` : `rgba(120,85,15,${a * .5})`;
...
for (let i = 0; i < 44; i++) nodes.push(new N());
anim();
...
function ctab(btn, pane) {
  ...
  document.getElementById('page-servicos').scrollTo({ top: 0, behavior: 'smooth' });
}
